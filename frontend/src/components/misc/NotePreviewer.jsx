...
export default function NotePreviewer({previewableContent, showPreview }) {
  ...
  return (
    <div className={`fixed grid grid-cols-12 grid-rows-12 w-[90svw] lg:w-[80svw] xl:w-[60svw] h-[80svh] top-1/2 left-1/2 -translate-x-1/2 px-7 pb-6 overflow-y-scroll rounded-md bg-white text-slate-800 dark:bg-slate-700 ${showPreview ? '-translate-y-1/2 z-30 opacity-100' : 'opacity-0 -translate-y-10 z-0'} transition-all duration-300`}>
      ...
      <h2 className={`fira-mono place-self-end col-start-7 col-end-13 row-start-1 row-end-2 font-semibold text-[18px] dark:bg-slate-700 dark:text-white`}>{calcDateTime(previewableContent.created_at)}</h2>
